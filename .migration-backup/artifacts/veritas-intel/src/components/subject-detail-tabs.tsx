...
export function SubjectDetailTabs({ subject }: { subject: Subject }) {
  ...
  type SherlockResult = { site: string; exists: boolean; url?: string };
  type BreachResult = { name: string; breachDate: string; dataClasses: string[] };
  type DeepSearchState = {
    error?: string;
    result?: {
      summary: string;
      findings: { platform: string; status: string; details: string; confidence: number }[];
  ...
  const [deepSearchState, deepSearchAction, isDeepSearching] = useActionState<DeepSearchState, FormData>(
    performDeepSearchAction.bind(null, plainSubject) as (prev: DeepSearchState, fd: FormData) => Promise<DeepSearchState>,
    {}
  );
