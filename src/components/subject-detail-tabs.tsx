...
export function SubjectDetailTabs({ subject }: { subject: Subject }) {
  ...
  return (
    <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            ...
            <CardHeader className="pb-4 border-b">
              <div className="flex items-center justify-between">
                <CardTitle className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground flex items-center gap-2">
                  <Fingerprint className="h-4 w-4 text-primary" /> Active Command Center
                </CardTitle>
            ...
            <CardContent className="space-y-8 pt-6">
              <div className="grid grid-cols-2 gap-8">
                <div className="flex flex-col gap-2">
                  <span className="text-[10px] uppercase font-bold text-muted-foreground tracking-widest">Verification Confidence</span>
                  <span className="text-6xl font-black tracking-tighter">{latestReport?.verificationScore || '0'}%</span>
                </div>
                <div className="text-right">
                  <span className="text-[10px] uppercase font-bold text-muted-foreground tracking-widest">Compliance Status</span>
                  <div className={`mt-2 text-2xl font-black px-4 py-1 uppercase border-2 ${subject.status === 'Clear' ? 'border-primary' : 'border-destructive text-destructive'}`}>
                    {subject.status}
                  </div>
                </div>
              </div>
              ...
              {mounted && chartData.length > 0 && (
                <div className="h-[120px] w-full border p-4 bg-muted/5">
              ...
              <div className="grid grid-cols-3 gap-4">
                  <div className="p-3 border bg-muted/5">
                    <p className="text-[8px] font-black uppercase text-muted-foreground mb-1 tracking-widest">Breach Hits</p>
                    <p className="text-xl font-black">{deepSearchState.result?.breachResults?.length || 0}</p>
                  </div>
                  <div className="p-3 border bg-muted/5">
                    <p className="text-[8px] font-black uppercase text-muted-foreground mb-1 tracking-widest">CIPC Registry</p>
                    <p className="text-xl font-black">{corporateData.length}</p>
                  </div>
                  <div className="p-3 border bg-muted/5">
                    <p className="text-[8px] font-black uppercase text-muted-foreground mb-1 tracking-widest">RICA Status</p>
                    <p className="text-xl font-black">{ricaData?.status === 'Verified' ? 'PASS' : 'NULL'}</p>
                  </div>
              </div>
            </CardContent>
          </Card>
                ...
                <div className="space-y-4 pr-4">
                  {mounted && auditLog?.map((log) => (
                    <div key={log.id} className="border-l border-white/20 pl-3 py-1">
                      <p className="text-[9px] font-black uppercase tracking-tight">{log.action}</p>
                      <span className="text-[7px] opacity-50 font-mono">
      ...
      <TabsContent value="osint" className="mt-6 space-y-6">
        <Card className="border-2 border-primary bg-background shadow-[8px_8px_0px_0px_rgba(0,0,0,0.1)]">
          <CardHeader className="border-b">
            <div className="flex items-center justify-between">
              <CardTitle className="text-[10px] font-black uppercase tracking-widest flex items-center gap-2">
                <Zap className="h-4 w-4 text-primary" /> Real-Time OSINT Gateway
              </CardTitle>
              <form action={deepSearchAction}>
                <Button size="sm" disabled={isDeepSearching} className="rounded-none font-black h-8">
            ...
            <div className="grid md:grid-cols-2 gap-8">
              <div className="space-y-6">
                 {deepSearchState.result && (
                   <div className="bg-black text-white p-6 font-mono text-[9px] h-64 overflow-auto border-l-8 border-primary">
                     <p className="text-primary mb-2"># VERITAS ACTIVE DISCOVERY V5.0</p>
                 ...
                 {deepSearchState.result ? (
                   <div className="p-6 bg-muted/10 border-l-4 border-primary italic text-sm">
                     "{deepSearchState.result.summary}"
                   </div>
                 ) : (
                   <div className="h-64 flex items-center justify-center border-2 border-dashed">
                     <p className="text-[10px] font-black uppercase opacity-30">Waiting for discovery trigger...</p>
                   </div>
                 )}
              </div>
              ...
              <div className="space-y-6">
                 <h4 className="text-[10px] font-black uppercase tracking-widest flex items-center gap-2">
                   <Database className="h-3 w-3" /> Digital Intelligence Points
                 </h4>
                 {deepSearchState.result?.breachResults?.map((breach, i) => (
                   <div key={i} className="p-4 border bg-background space-y-2">
                      <div className="flex justify-between items-center">
                        <span className="font-black text-xs uppercase">{breach.name}</span>
                        <Badge variant="destructive" className="text-[8px]">LEAKED</Badge>
                      </div>
                      <div className="flex flex-wrap gap-1">
                        {breach.dataClasses.map((c, j) => (
                          <Badge key={j} variant="outline" className="text-[7px] uppercase">{c}</Badge>
                        ))}
                      </div>
                   </div>
                 ))}
                 ...
                 <div className="pt-4 border-t space-y-4">
                    ...
                    <div className="p-4 border bg-muted/5 font-mono text-[9px] space-y-1">
      ...
      <TabsContent value="profile" className="mt-6">
        <Card className="border-2 border-primary bg-background">
          <CardHeader className="border-b">
            <CardTitle className="text-lg font-black uppercase tracking-widest">Metadata Repository</CardTitle>
          </CardHeader>
          <CardContent className="grid md:grid-cols-2 gap-12 pt-8 pb-12">
            <div className="space-y-8">
              <div>
                <Label className="text-[10px] font-black uppercase opacity-50">Full Name</Label>
                <p className="text-3xl font-black uppercase tracking-tighter">{subject.name}</p>
              </div>
              <div>
                <Label className="text-[10px] font-black uppercase opacity-50">National Identity Number</Label>
                <p className="text-2xl font-mono font-black">{subject.idNumber}</p>
              </div>
            </div>
            <div className="space-y-8">
              <div>
                <Label className="text-[10px] font-black uppercase opacity-50">RICA Registered Number</Label>
                <p className="text-2xl font-mono font-black text-primary">{subject.phoneNumber}</p>
              </div>
              <div>
                <Label className="text-[10px] font-black uppercase opacity-50">Identity Verification</Label>
                <div className="mt-2 flex gap-2">
                  <Badge className="rounded-none bg-primary text-primary-foreground font-black px-4">DHA_VERIFIED</Badge>
                  <Badge variant="outline" className="rounded-none font-black border-primary">RICA_ACTIVE</Badge>
                </div>
              </div>
            </div>
          </CardContent>
        ...
        <Card className="border-2 border-primary mb-6">
           <CardContent className="pt-6 flex items-center justify-between">
             <div className="flex items-center gap-6">
               <div className={`p-4 border-2 ${simulating ? 'bg-primary animate-pulse' : 'border-primary'}`}>
                 <Radio className={simulating ? 'text-white' : 'text-primary'} />
               </div>
               <div>
                 <h3 className="font-black text-lg uppercase tracking-tighter">GSM Vector Intercept</h3>
                 <p className="text-[10px] font-bold text-muted-foreground uppercase">NODE: {subject.idNumber}</p>
               </div>
             </div>
             <Button size="lg" onClick={simulatePing} disabled={simulating} className="h-14 px-10 font-black rounded-none">
